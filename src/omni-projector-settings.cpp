...
void OmniProjectorSettings::Save(
    const std::vector<MappingEntry> &mappings)
{
    ...
    // 儲存至 OBS 的插件設定目錄
    char *bin_path = obs_module_get_config_path(
        obs_current_module(), "settings.json");
...
std::vector<MappingEntry> OmniProjectorSettings::Load()
{
    ...
    if (data) {
        obs_data_array_t *array =
            obs_data_get_array(data, "mappings");
        size_t count = obs_data_array_count(array);

        for (size_t i = 0; i < count; i++) {
            obs_data_t *obj = obs_data_array_item(array, i);
            mappings.push_back(
                {obs_data_get_string(obj, "source"),
                 (int)obs_data_get_int(obj, "monitor")});
            obs_data_release(obj);
        }
