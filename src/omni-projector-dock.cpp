...
OmniProjectorDock::OmniProjectorDock(QWidget *parent) : QDockWidget(parent)
{
    ...
    // 頂部：添加按鈕
    QPushButton *addBtn =
        new QPushButton("＋ 添加投影對應 (Add Mapping)");
    ...
    QPushButton *projectAllBtn =
        new QPushButton("全部開啟 (Project All)");
    QPushButton *stopAllBtn =
        new QPushButton("一鍵全關 (Close All)");
...
void OmniProjectorDock::RefreshMappings()
{
    ...
    for (int i = 0; i < (int)mappings.size(); ++i) {
        ...
        sourceCombo->setCurrentText(
            QString::fromStdString(entry.source_name));

        QComboBox *monitorCombo = new QComboBox();
        for (int m = 0; m < monitorCount; ++m) {
            QString name = screens[m]->name();
            monitorCombo->addItem(
                QString("[%1] %2").arg(m).arg(name));
        }
        ...
        // 更新與存檔事件
        connect(sourceCombo, &QComboBox::currentTextChanged,
                [i, sourceCombo, monitorCombo]() {
                    OmniProjectorManager::Get().UpdateMapping(
                        i,
                        sourceCombo->currentText().toStdString(),
                        monitorCombo->currentIndex());
                });

        connect(monitorCombo,
        ...
        connect(goBtn, &QPushButton::clicked,
                [sourceCombo, monitorCombo]() {
                    ...
                    if (source) {
                        OmniProjectorManager::Get().StartProjection(
                            source, mIdx);
