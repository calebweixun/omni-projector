...
- **FR-005**: 插件 MUST 定義翻譯來源策略（採用合併策略）：在 runtime 中若 OBS 提供對應翻譯鍵則優先使用 OBS 原生翻譯；若 OBS 未提供該鍵，則使用插件內建翻譯。插件應允許管理員覆寫此行為（例如在設定中強制以插件翻譯為準）。

- **FR-005a (維護責任)**: 插件 MUST 採用混合維護策略：插件 repo 維護插件專屬字串與未被 OBS 覆蓋的翻譯；OBS upstream 維護通用或平台字串。雙方的同步與衝突解決策略需在維運流程中明確化（例如定期比對與自動化同步檢查）。

- **FR-005b (缺漏回報)**: 插件 MUST 提供可匯出的缺漏翻譯清單（local report/export）以便翻譯者或維運人員追蹤並補齊；此清單為預設行為（不自動上傳遠端），並可在設定中選擇其他回報方式。
